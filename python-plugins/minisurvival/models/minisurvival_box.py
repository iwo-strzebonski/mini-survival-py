...
from mcapi import asynchronous, location, teleport, yell, getblock, setblock  # type: ignore
from org.bukkit import Material, Location  # type: ignore
...
class MinisurvivalBox:
    ...
    def __str__(self):
        ...
        return "MinisurvivalBox(width={}, height={}, length={}, x={}, y={}, z={}, players={})".format(
            self.__width,
            self.__height,
            self.__length,
            self.__x,
    ...
    def __run_commands(self, queue):
        # type: (Union[chain[Callable], Generator[Callable, Any, Any]]) -> None
    ...
    def prepare(self):
        ...
        yell(
            "Calculated size of the box: {}x{}x{}, preparing arena...".format(
                self.__width, self.__height, self.__length
            )
        )
        ...
        queue = chain.from_iterable(self.__create_door())
        self.__run_commands(queue)

        yell("Exit door generated, game is ready!")

    def __build_box(self):
        # type: () -> Generator[list[Callable], Any, Any]
        ...
        yield [
            partial(
                ...
                cuboid(
                    ...
                    width=self.width(),
                    height=round(self.height() * self.__GROUND_HEIGHT_MULTIPLIER),
                    length=self.length(),
    ...
    def __teleport_players(self, offset=0):
        # type: (int) -> Generator[list[Callable], Any, Any]
        ...
        for player in self.__players:
            yield [
                partial(yell, "Teleporting {} to the box...".format(player.getName())),
                partial(
                    teleport,
    ...
    def __generate_layers(self):
        # type: () -> Generator[Callable, Any, Any]
    ...
    def __generate_trees(self):
        ...
        yield partial(
            dispatch_command,
            cuboid(
                location(self.__x, self.__y + self.__height, self.__z),
                width=4,
                height=4,
                length=4,
                type=Material.AIR,
